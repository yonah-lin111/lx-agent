...
describe("SystemPromptManager", () => {
  ...
  describe("默认单例与内置分层 (createDefaultSystemPromptManager)", () => {
    it("包含基础身份、行为层、Persona 及动态技能与指令分层", async () => {
      ...
      expect(assembly.rendered).toContain("## Ambition vs Precision")
      expect(assembly.rendered).toContain("## Task Execution & Editing Constraints")
      ...
      expect(assembly.rendered).toContain("## Safety Boundary & Dirty Worktrees")
