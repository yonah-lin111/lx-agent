...
describe("我的世界主题复选框与单选项", () => {
  it("未选中态为像素方格：圆角归零、2px 黑边、凹底与右下硬阴影", () => {
    const block = getRuleBlock('[data-theme="pixel"] .lx-checkbox-box,')
  ...
  it("选中态使用经验条荧光绿底、黑边与外发光", () => {
    const block = getRuleBlock(
      '[data-theme="pixel"] .lx-checkbox input:checked + .lx-checkbox-box,',
    )
  ...
  it("悬停提亮内高光，键盘聚焦叠加绿色微光", () => {
    const hover = getRuleBlock('[data-theme="pixel"] .lx-checkbox:hover .lx-checkbox-box,')
    expect(hover).toContain("rgba(255, 255, 255, 0.3)")

    const focus = getRuleBlock(
      '[data-theme="pixel"] .lx-checkbox input:focus-visible + .lx-checkbox-box,',
    )
    expect(focus).toContain("0px 0px 6px rgba(85, 255, 85, 0.35) !important")
  })

  it("单选项行容器圆角归零", () => {
    const block = getRuleBlock('[data-theme="pixel"] .lx-radio {')
    expect(block).toContain("border-radius: 0px !important")
  })

  it("单选项选中内点直角化，统一像素语言", () => {
    const block = getRuleBlock('[data-theme="pixel"] .lx-radio-dot::before {')
    expect(block).toContain("border-radius: 0px !important")
  })
})
