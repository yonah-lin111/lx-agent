...
describe("我的世界主题游戏视图", () => {
  it("区块头退化为纯布局行，不再叠加马赛克容器", () => {
    const block = getRuleBlock('[data-theme="pixel"] .game-dashboard > section')
  ...
  it("标题与图标对齐索引页的像素描边语言", () => {
    expect(getRuleBlock('[data-theme="pixel"] .game-section-title')).toContain(
      "text-shadow: 1px 1px 0px #000000",
    )
    expect(getRuleBlock('[data-theme="pixel"] .game-section-icon')).toContain(
      "drop-shadow(1px 1px 0px #000000)",
    )
    expect(getRuleBlock('[data-theme="pixel"] .game-section-icon--game')).toContain(
      "color: #ffaa00 !important",
    )
  })

  it("卡片与工具栏图标使用像素投影，并按分区着色", () => {
    const block = getRuleBlock('[data-theme="pixel"] .game-card-icon,')

    expect(block).toContain("drop-shadow(1px 1px 0px #000000)")

    expect(getRuleBlock('[data-theme="pixel"] .game-card-icon--builtin,')).toContain(
      "color: #55ff55 !important",
    )
    expect(getRuleBlock('[data-theme="pixel"] .game-card-icon--imported')).toContain(
      "color: #ffaa00 !important",
    )
  })

  it("游戏卡片使用与内置游戏卡片一致的像素浮雕", () => {
    const block = getRuleBlock('[data-theme="pixel"] .game-card {')
    ...
    const hoverBlock = getRuleBlock('[data-theme="pixel"] .game-card:hover')
    expect(hoverBlock).toContain("background-color: #242434 !important")
  })

  it("内置游戏卡片沿用相同的像素浮雕容器", () => {
    const block = getRuleBlock('[data-theme="pixel"] .builtin-game-card {')

    expect(block).toContain("border: 2px solid #000000 !important")
    expect(block).toContain("background-color: #1e1e2a !important")

    const hoverBlock = getRuleBlock('[data-theme="pixel"] .builtin-game-card:hover')
    expect(hoverBlock).toContain("background-color: #242434 !important")
  })

  it("内置 / 导入分区图标使用像素描边与各自主题色", () => {
    expect(getRuleBlock('[data-theme="pixel"] .game-section-icon--builtin')).toContain(
      "color: #55ff55 !important",
    )
    expect(getRuleBlock('[data-theme="pixel"] .game-section-icon--imported')).toContain(
      "color: #ffaa00 !important",
    )
  })

  it("空态占位与模拟器视口保留像素凹槽", () => {
    const emptyBlock = getRuleBlock('[data-theme="pixel"] .game-empty-state')
    ...
    const viewportBlock = getRuleBlock('[data-theme="pixel"] .game-stage-viewport')
