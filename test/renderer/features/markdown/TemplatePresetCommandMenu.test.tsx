...
describe("TemplatePresetCommandMenu", () => {
  ...
  it("正常渲染全部预设选项并正确标记当前激活项", () => {
    ...
    expect(options[1].textContent).toContain("Add Requirement")
  })
  ...
  it("点击选项时触发 onSelect 回调并回传对应预设数据", () => {
    ...
    expect(calledWith.content).toContain("Fix runtime error")
  })
  ...
  it("通过 useMarkdownPanels 打开面板、上下键切换、选中并严格顶格输出（移除缩进）", () => {
    const doc = "$$$\n  /templatePreset\n$$$"
    ...
    // 3. 选中当前项（addOption），验证严格顶格插入（移除原本的 2 空格缩进）
    const addOption = MARKDOWN_TEMPLATE_PRESET_OPTIONS[1]
    act(() => {
      result.current.selectTemplatePreset(addOption)
    })
