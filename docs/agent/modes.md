...
- **切换入口**：`Shift + Tab` 在 `build → auto → plan → review → design → minimal → build` 循环；状态栏 `CollaborationModeButton` 同步展示（auto 下显示「Auto · 有效模式」），点击标签弹出模式列表可定向切换；卡片一键采纳经 `setEffectiveMode` 退出只读模式；新会话启动模式取 `agent.permissions.collaborationMode`（缺省 `build`）。
