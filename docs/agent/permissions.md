...
- 非 build 模式的 deny 为**硬拦截**：不进入审批弹窗，直接返回带模式说明的 error ToolResult 回灌模型（`MODE_MUTATION_REASONS`）；`memory` 会写 `<project>/.lx/memory/memory.xml` 与 `~/.lx/memory/memory.xml`，因此同样纳入基线。`plan` 的 `question` 使用专用 reason（`PLAN_QUESTION_TOOL_REASON`）：提示模型按内嵌 grill-me 协议用纯文本一次一问（`问题: / 推荐: / 推荐举例说明:`，标签随用户语言本地化）。
