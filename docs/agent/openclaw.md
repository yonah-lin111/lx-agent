...
| `/only` | 只显示选中员工的消息（空格多选实时过滤，回车/Esc 收起；需办公区 ≥ 2 名员工） |
| `/all` | 恢复显示全部员工消息（仅筛选态下可见） |
| `/stop` | 中止当前办公区内所有流式任务 |
| `/office` | 打开办公区切换面板 |

按键：`↑/↓` 移动、`Enter` 选择、`Esc` 关闭/中止流式；无面板时 `Enter` 发送、`Shift+Enter` 换行。文本派生的二级面板（`/clear`、`/only`）按 `Esc` 会连同派生文本一并丢弃。员工多选通过左栏名册或 `@claw:` 提及完成；消息列表筛选不影响派发目标，顶栏提供筛选 chips 与恢复按钮。
