      ...
      {hasError ? (
        ...
        <div className="grid min-h-0 grid-cols-2 gap-1.5">
          {stats.map((stat) => (
            <div
              key={stat.label}
              className="flex min-w-0 items-baseline justify-between gap-2 rounded-[var(--theme-radius-base)] border border-[var(--color-theme-border)] bg-[var(--color-theme-surface-hover)] px-2 py-1.5"
            >
              <span className="truncate text-[11px] text-[var(--color-theme-text-muted)]">
                {stat.label}
              </span>
              <span className="shrink-0 font-mono text-[13px] font-medium text-[var(--color-theme-text)]">
                {stat.value}
              </span>
            </div>
          ))}
