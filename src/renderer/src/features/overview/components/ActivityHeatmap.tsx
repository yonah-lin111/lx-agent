...
/**
 * 渲染生产力绿墙热力图：
 * 1. 宽度足够时全月单行平铺，宽度不足时逐月自适应折行；
 * 2. 移除数百个内联 Tooltip 实例，采用全局单例悬浮 Portal 气泡，彻底根治渲染卡顿；
    ...
    <div className="overview-heatmap-card flex min-w-0 flex-col gap-4 rounded-[6px] border border-white/5 bg-[#262626] p-4">
      ...
      <div className="flex items-start gap-2 min-w-0 w-full">
        ...
        <div className="flex flex-wrap items-start gap-x-3 gap-y-3.5 min-w-0 flex-1">
          {months.map((month) => (
            <div key={month.monthKey} className="flex flex-col items-center gap-1.5 shrink-0">
              {/* 月份名称 */}
      ...
      {/* 全局单例悬浮 Tooltip 气泡（零常驻内存开销） */}
      {activeTooltip &&
        tooltipContent &&
        createPortal(
          <div
            role="tooltip"
            className="fixed z-[999999] pointer-events-none rounded-[6px] bg-[#303030] px-2.5 py-1.5 text-xs font-semibold text-white whitespace-normal break-words drop-shadow-[0_4px_12px_rgba(0,0,0,0.5)] select-none"
            style={{
