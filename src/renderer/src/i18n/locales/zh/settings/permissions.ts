...
- **生效时机**：下一轮对话生效；硬基线工具在保存时自动从白名单中剥离。`,
  ...
  customCommandBlocksList: "模板块列表",
  customCommandBlockTemplateName: "任务块",
  customCommandBlockSuppleName: "临时块",
  customCommandBlockLogName: "记录块",
  customCommandBlockTemplateScope: "全局可用",
  customCommandBlockSubblockScope: "仅任务块内部",
  customCommandBlockTemplateDesc:
    "文档主体块：承载任务说明与需求，支持标题、状态循环与工作区绑定；插入时自动补全唯一 id。",
  customCommandBlockSuppleDesc: "临时补充块：记录补充需求，复制父模板时整块剔除，不沉淀到模板。",
  customCommandBlockLogDesc: "执行记录块：记录执行过程与结论，复制父模板时保留内容。",
  customCommandBlockSyntaxTitle: "语法",
  customCommandBlockSyntaxStartEnd:
    "块以 --start 与 --end 标记首尾，名称可为任意标识符（如 xxxTemplate）。",
  customCommandBlockSyntaxTitleField: "开始行可携带 「title: 标题」，显示在块头部。",
  customCommandBlockSyntaxId: "结束行插入时自动补全 {id:...} 唯一标识（编辑器内只读）。",
  customCommandBlockSyntaxWt: "任务块与临时块结束行支持 {wt:分支} 工作区绑定。",
  customCommandBlockSyntaxNested: "临时块与记录块只能嵌套在任务块内部使用。",
  customCommandBlockExampleTitle: "示例",
