...
// 模拟器宿主页 CSP：仅放行本协议资产与 WASM 编译，不放开主窗口防线。
// EmulatorJS 解包 7z 核心（压缩包解压器使用 eval）与经 blob: 装载核心脚本/实例化 wasm，
// 因此 guest 页必须放行 'unsafe-eval' 与 blob:；页面本身只加载同源本地资产，无任何外部来源。
const WRAPPER_CSP = [
  "default-src 'none'",
 ...
 */
const createGameRequestHandler = (): ((request: Request) => Response) => (request) => {
  try {
...
export const registerGameProtocol = (): void => {
  ...
  protocol.handle(GAME_PROTOCOL, handler)
  session.fromPartition(GAME_WEBVIEW_PARTITION).protocol.handle(GAME_PROTOCOL, handler)
}
