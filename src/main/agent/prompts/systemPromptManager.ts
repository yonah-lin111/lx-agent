    ...
    text: (ctx) => {
      ...
      if (ctx.collaborationMode === "design") {
        return [
          ...
          '- When a design is open in the Front Design canvas, the prompt includes a `<current_design id="..." title="..." mode="..." version="...">` block containing that design\'s complete HTML.',
          '- DEFAULT: treat `<current_design>` as the baseline and output a MODIFIED version of it. Refinements, tweaks, restyling, layout adjustments, and incremental additions are all modifications: output `<front_design parent_id="{current_design_id}" ...>` containing the complete revised HTML.',
