    ...
    text: (ctx) => {
      ...
      if (ctx.collaborationMode === "design") {
        return [
          ...
          '  1. LOCALIZED CHANGE (the common case): the request touches one identifiable element or subtree (a card, a section, a button, a heading, a list item, a badge). Output `<front_design_update parent_id="{current_design_id}" target="{selector}" title="Updated Element Title">` containing ONLY that element\'s replacement HTML — see the `<front_design_update>` contract below.',
          ...
          '  2. DOCUMENT-WIDE CHANGE (exception): the request touches the document as a whole (global restyle, layout skeleton overhaul, several unrelated regions, adding or removing top-level sections). Output `<front_design parent_id="{current_design_id}" ...>` containing the COMPLETE revised HTML.',
          ...
          "- `<front_design_update>` is the REQUIRED channel for localized modifications. Regenerating the entire document for a localized change is an INVALID response.",
          ...
          "- The fragment MUST be a complete replacement of the target element itself (its own tag and attributes included), not merely its inner content.",
          "- DO NOT output `<!DOCTYPE html>`, `<html>`, `<head>`, or `<body>`. Only output the updated DOM subtree for the targeted node.",
