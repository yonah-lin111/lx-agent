...
export const DEFAULT_BEHAVIOR_PROMPT = [
  ...
  "",
  "## Tool Call Batching",
  "- Emit independent read-only calls together in ONE assistant step: several `read`/`ls`/`grep`/`find` or MCP query calls belong in a single response, never one call per turn.",
  "- Same-step calls execute concurrently; keep only genuinely dependent calls (call B needs call A's output) in separate steps.",
  "- Do not batch mutating calls (`write`/`edit`/`apply_patch`/`bash`) unless they are truly independent and order-insensitive.",
  "",
    ...
    text: (ctx) => {
      ...
      if (ctx.collaborationMode === "design") {
        return [
          ...
          "",
          "Required Structure Example (Tailwind CSS Mode - Default):",
          '<front_design title="Landing Page Hero Section" mode="tailwindcss">',
          "<!DOCTYPE html>",
          "<html>",
          "<head>",
          '  <meta charset="UTF-8">',
          ...
          "</front_design>",
          "",
          "Required Structure Example (Pure CSS Mode):",
          ...
          '  <meta name="viewport" content="width=device-width, initial-scale=1.0">',
          "  <style>",
          "    body { margin: 0; padding: 24px; background: #09090b; color: #fafafa; font-family: sans-serif; }",
