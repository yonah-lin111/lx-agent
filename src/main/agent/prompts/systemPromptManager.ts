    ...
    text: (ctx) => {
      if (ctx.collaborationMode === "plan") {
        return [
          ...
          "- Not allowed (mutating): Editing or writing files (edit, write, apply_patch, todowrite, memory), running commands that modify repo-tracked state.",
          "",
      ...
      if (ctx.collaborationMode === "review") {
        return [
          ...
          "- You are strictly in read-only analysis. Mutating tools (`write`, `edit`, `apply_patch`, `todowrite`, `memory`) are strictly disabled.",
          "- Sub-agent dispatch (`task`) is limited to the roles listed in the task tool's `Available agent types` for this mode (read-only `explorer` by default). Unlisted roles, and roles whose capability set includes tools blocked by this mode, are rejected.",
          ...
          "- When the user requests a review without specifying a target, default to the current uncommitted changes (staged, unstaged, and untracked files).",
          "- Inspect the target yourself with read-only commands (e.g. `git status`, `git diff HEAD`) before producing findings.",
          "",
      ...
      if (ctx.collaborationMode === "design") {
        return [
          "# Collaboration Mode: Front Design Mode",
          "",
          ...
          "- Mutating tools (`write`, `edit`, `apply_patch`, `todowrite`, `memory`) are disabled in this mode.",
          "- Sub-agent dispatch (`task`) is limited to the roles listed in the task tool's `Available agent types` for this mode (read-only `explorer` by default). Unlisted roles, and roles whose capability set includes tools blocked by this mode, are rejected.",
          "",
