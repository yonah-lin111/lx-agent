...
export const GRILL_ME_SKILL_PROMPT = [
  ...
  "    Every question turn MUST wrap exactly ONE grill-question block in `<grill_question>` and `</grill_question>` tags, with the labeled lines inside and nothing else in the block:",
  "",
  "    <grill_question>",
  "    问题: <one concrete, scoped question about a single decision; answerable by choosing an option key or by a short sentence>",
  "    A) <first concrete option>",
  "    B) <second concrete option>",
  "    推荐: <recommended option key or answer, plus one brief reason>",
  "    推荐举例说明: <a concrete, everyday example that makes the recommendation immediately understandable>",
  "    </grill_question>",
  "",
  "    Option rules:",
  "    - When the decision has discrete choices, list each choice on its own line right after the question line, keyed `A) `, `B) `, `C) ` … (capital letter, half-width parenthesis, one option per line).",
  "    - NEVER inline choices inside the question sentence, e.g. `(A) ... (B) ... (C) ...` prose is forbidden. The client renders option lines as selectable rows; inline choices render as an unreadable paragraph.",
  "    - Keep 2-5 options; put `其他（请具体描述）` / `Other (please specify)` last only when truly needed.",
  "    - When the decision is open-ended (a number, a name, a free description), omit the option lines entirely.",
  "    - The `推荐:` line MUST name the recommended option key first (e.g. `推荐: A——...`), then the brief reason.",
  "    - Keep the question stem on the `问题:` line and the options on their own lines; never put an option on the `推荐:` or `推荐举例说明:` lines.",
  "",
  "    Language contract:",
  ...
  "",
  "    Tag contract (the client renders these tags as a decision card):",
  "    - Keep the tag names exactly `<grill_question>` and `</grill_question>` in every language; never translate or rename them.",
  "    - The opening tag MUST be on its own line, the closing tag MUST be on its own line, and each label line MUST start at the beginning of its line.",
  "    - Put the whole question (question + recommendation + example) inside the tags; never leave one of the three lines outside the block.",
  "    - A turn asking a question contains exactly one `<grill_question>` block. Never batch two questions into one block or repeat the block in the same turn.",
  "  </output_format>",
  "  <flow>",
